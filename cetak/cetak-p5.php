...
$namafile=$rombs['rombel']."-".$siswa['nama'].".pdf";
 ...
 $table2->easyCell('Proyek '.$smt.'. '.$npro['nama_proyek'], 'border:LTR;align:L;');
 $table2->printRow();
 $table2->rowStyle('font-size:12;');
 $table2->easyCell($npro['deskripsi_proyek'], 'border:LBR;align:J;');
 $table2->printRow();
 $table2->endTable();
...
while($s=$query->fetch_assoc()) {
	...
	$rapo->rowStyle('font-size:12; font-style:B; bgcolor:#BEBEBE;min-height:8');
	...
	$promin=$connect->query("select * from penilaian_proyek where peserta_didik_id='$idp' and kelas='$kelas' and tapel='$tapel' and smt='$smt' and dimensi='$iddimensi' order by nilai asc")->fetch_assoc();
	$promax=$connect->query("select * from penilaian_proyek where peserta_didik_id='$idp' and kelas='$kelas' and tapel='$tapel' and smt='$smt' and dimensi='$iddimensi' order by nilai desc")->fetch_assoc();
	...
	$rapo->rowStyle('font-size:12; min-height:35');
...
$table6=new easyTable($pdf, '{20,100}', 'align:L');
$table6->rowStyle('font-size:12; font-style:B;');
$table6->easyCell('Keterangan Asesmen :','colspan:2;align:L; valign:M');
$table6->printRow();
$table6->rowStyle('font-size:10;');
$table6->easyCell('BB');
...
$pdf = new Fpdi();
// get the page count
$pageCount = $pdf->setSourceFile('rapor-p5.pdf');
...
for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
    ...
    $pdf->SetXY(10, 270);
    $pdf->Write(5, $siswa['nama']);
	$pdf->SetXY(165, 270);
    $pdf->Write(5, 'Halaman '.$pageNo.' dari '.$pageCount);
}
