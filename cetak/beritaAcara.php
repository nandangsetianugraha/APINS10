...
class PDF extends FPDF{
  ...
  function Header(){
    ...
    $this->Ln(0.5);
    $this->Cell(18,0.8,strtoupper($Ls['jenis']),0,0,'C');
    $this->Ln(0.5);
    $this->Cell(18,0.8,'TAHUN PELAJARAN '.$Ls['tapel'],0,0,'C');
    $this->Ln(1.5);
    $this->SetFont('Arial','',10);
    $this->MultiCell(18,0.5,'Pada hari ini '.namahari($tanggal).' tanggal '.penyebut($tgl).' bulan '.namaBulan($bln).' tahun '.penyebut($thn).', di SD Islam Al-Jannah telah diselenggarakan '.$Ls['jenis'].', untuk Mata Pelajaran '.$Ls['mapel'].' dari pukul '.$Ls['mulai'].' sampai dengan pukul '.$Ls['selesai'],0,'J',0);
    $this->Ln(0);
