...
$(document).ready(function(){	
	...
	$('#kd').change(function(){
			...
			$.ajax({
				type : 'GET',
				url : 'modul/penilaian/NilaiPAS.php',
				data :  'mp=' + mp+'&kelas='+kelas+'&smt='+smt+'&peta='+peta+'&tapel='+tapel+'&kd='+kd,
				beforeSend: function()
				{	
					$("#loading").show();
					$(".loader").show();
					$("#nilaiHarian").html('<div class="alert alert-info alert-dismissible"><h4><i class="fa fa-spinner fa-pulse fa-fw"></i> Memuat Data Nilai PAS....</h4></div>');
				},
				success: function (data) {
					$("#loading").hide();
					$(".loader").hide();
