...
$(document).ready(function() {
	...
	$('#kelas').change(function(){
			...
			$.ajax({
				...
				success: function (data) {
					...
					$("#mp").html(data);
					$("#materi").html('<select class="form-control" id="materi" name="materi"><option value="0">Pilih Materi</option></select>');
					var kelas = $('#kelas').val();
					var materi = $('#materi').val();
					var smt = $('#smt').val();
					var mp = $('#mp').val();
	...
	$('#addProyek').on('show.bs.modal', function (e) {
			...
			var smt = $('#smt').val();
			var materi = $('#materi').val();
			if(kelas==0 || mapel==0 || materi==0){
				Swal.fire("Kesalahan","Isi Dahulu Kelas Mapel dan Materi","error");
			}else{
			//menggunakan fungsi ajax untuk pengambilan data
            $.ajax({
                type : 'post',
                url : 'modul/administrasi/m_tujuan.php',
                data :  'kelas='+ kelas+"&smt="+smt+"&mapel="+mapel+'&materi='+materi,
				beforeSend: function()
						{	
							$("#loading").show();
							$(".loader").show();
