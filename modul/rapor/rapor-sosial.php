...
while($s=$query->fetch_assoc()) {
	...
	if($ada>0){
		...
		$actionButton,
		$j['nama'],
		$deskripsi.' <button class="btn btn-info btn-border btn-round btn-sm" type="button" id="'.$idr.'" data-bs-toggle="modal" data-bs-target="#modalekskul" data-id="'.$idr.'"><i class="fas fa-edit"></i></button>'		
		);
	}else{
		$output['data'][] = array(
		$actionButton,
